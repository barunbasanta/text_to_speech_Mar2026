...
def is_script_char(ch: str) -> bool:
    ...
    return category.startswith(("L", "M"))


def contains_english_char(text: str) -> bool:
    return any(is_english_char(ch) for ch in text)


def contains_non_english_script(text: str) -> bool:
    return any(is_script_char(ch) for ch in text)
...
def split_mixed_text(text: str) -> list[tuple[str, str]]:
    ...
    return [(seg_type, seg_text.strip()) for seg_type, seg_text in merged if seg_text.strip()]


def should_use_mixed_logic(language_group: str, text: str) -> bool:
    return (
        language_group != "English (US)"
        and contains_non_english_script(text)
...
def concat_mp3_files(segment_paths: list[str], output_path: str) -> None:
    ...
    try:
        ...
        if copy_result.returncode != 0:
            recode_cmd = [
                "ffmpeg", "-y", "-f", "concat", "-safe", "0",
                "-i", list_path, "-acodec", "libmp3lame", "-q:a", "2", output_path
            ]
            recode_result = subprocess.run(recode_cmd, capture_output=True, text=True)
            if recode_result.returncode != 0:
                raise RuntimeError(recode_result.stderr or copy_result.stderr or "ffmpeg concat failed")
    finally:
...
if "rate_percent" not in st.session_state or not st.session_state.use_custom_controls:
    st.session_state.rate_percent = default_rate

if "pitch_hz" not in st.session_state or not st.session_state.use_custom_controls:
