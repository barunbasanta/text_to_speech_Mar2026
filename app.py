...
current_voices = VOICE_OPTIONS[st.session_state.language_group]
if "selected_voice" not in st.session_state or st.session_state.selected_voice not in current_voices:
    st.session_state.selected_voice = list(current_voices.keys())[0]

if "style_preset" not in st.session_state:
    st.session_state.style_preset = "Natural"
...
def language_changed() -> None:
    ...
    voice_map = VOICE_OPTIONS[language]
    st.session_state.selected_voice = list(voice_map.keys())[0]


# Widgets outside form so dependent widgets update immediately
st.selectbox(
    "Language",
