...
# Each language now maps voice categories to actual voice ids.
# Labels shown to the user are only category names.
VOICE_OPTIONS = {
    "English (US)": {
...
# Presets are shown conditionally based on the selected voice category.
...
async def synthesize_to_file(text: str, voice: str, rate: str, pitch: str, output_path: str) -> None:
    communicate = edge_tts.Communicate(text=f'<speak>{text}</speak>', voice=voice, rate=rate, pitch=pitch)
...
if submitted:
    cleaned_text = convert_pause_to_ssml(text.strip())
