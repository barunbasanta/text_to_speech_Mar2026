...
with st.form("tts_form"):
    text = st.text_area(
        "Enter text",
        value="So, today's story. So today I would like to tell you a story. Let's see how it goes.",
        height=180,
        placeholder="Type or paste text here...",
    )

    language_group = st.selectbox("Language", list(VOICE_OPTIONS.keys()), index=0)
    voice_map = VOICE_OPTIONS[language_group]
    ...
    if use_custom_controls:
        ...
        st.write(f"Speed: {rate_percent:+d}%")
        st.write(f"Pitch: {pitch_hz:+d}Hz")

    filename = st.text_input("Output filename", value="speech.mp3")
    submitted = st.form_submit_button("Generate Speech")

if selected_voice in VOICE_STYLE_HINTS:
    st.info(VOICE_STYLE_HINTS[selected_voice])

st.caption(
    "Note: voice providers usually offer named male/female voices, not exact age controls. "
    "Style presets help approximate common voices such as storyteller, cartoon, deep male, or a mature Bengali narrator."
)
